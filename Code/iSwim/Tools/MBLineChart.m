...
- (void)initGraph:(NSString *)type yValues:(NSArray *)yValues xValues:(NSArray *)xValues
{
    ...
    if (yValues.count > 10)
    {
        yValues = [yValues subarrayWithRange:NSMakeRange(0, 10)];
        xValues = [xValues subarrayWithRange:NSMakeRange(0, 10)];
    }
...
+ (UIScrollView *)initGraph:(NSString *)type yValues:(NSArray *)yValues xValues:(NSArray *)xValues inView:(UIView *)view
{
    ...
    //initate the graph view
    SHLineGraphView *_lineGraph = [[SHLineGraphView alloc] initWithFrame:graphFrame];
    //set the main graph area theme attributes
    /**
     *  theme attributes dictionary. you can specify graph theme releated attributes in this dictionary. if this property is
     *  nil, then a default theme setting is applied to the graph.
    ...
    [scrollView setContentSize:_lineGraph.frame.size];
